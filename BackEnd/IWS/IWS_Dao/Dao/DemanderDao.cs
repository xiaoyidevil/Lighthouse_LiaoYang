...
namespace IWS_Dao.Dao
{
    ...
    /// 需求方数据库操模型作类
    /// </summary>
    public class DemanderDao : AbstractDao<m_demander>, InterfaceDao<m_demander>
    {
        #region 接口函数
        ...
        public int InsertData(MySqlConnection conn, Dictionary<string, string> dicCondition, List<m_demander> lstData = null)
        {
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    ...
                    // 循环执行插入语句
                    foreach (m_demander demander in lstData)
                    {
                        paras = CreateInsertParameter();
                        paras[0].Value = demander.DemanderId;
                        paras[1].Value = demander.CompanyName;
                        paras[2].Value = demander.CompanyAddress;
                        paras[3].Value = demander.PostCode1;
                        paras[4].Value = demander.PostCode2;
                        paras[5].Value = demander.Website;
        ...
        {
            // 返回集合
            List<m_demander> lstDemander = new List<m_demander>();

            // 连接失效返回空
            if (conn.State != System.Data.ConnectionState.Open) return lstDemander;
        ...
        public override MySqlParameter[] CreateInsertParameter()
        {
            ...
            {
                new MySqlParameter("@DemanderId",MySqlDbType.VarChar,10),
        ...
        public override string CreateInsertSql(Dictionary<string, string> dicCondition)
        {
            ...
            sb.Append(" insert into m_demander ");
            sb.Append(" ( ");
            sb.Append("         DemanderId, ");
        ...
        public override string CreateUpdateSql(Dictionary<string, string> dicCondition)
        {
            ...
            sb.Append(" update m_demander ");
            sb.Append("    set DemanderId = @DemanderId, ");
